...
function AdminSidebar({ collapsed = false, mobileOpen = false, onToggle, onClose }) {
    ...
    const menuItems = [
        ...
        { name: "Courses", path: "/admin/courses", icon: "bi-book" },
        { name: "Lessons", path: "/admin/lessons", icon: "bi-journal-text" },
        { name: "Users", path: "/admin/users", icon: "bi-people" },
        { name: "Career Roadmaps", path: "/admin/create-roadmap", icon: "bi-signpost-split" },
        { name: "Internships", path: "/admin/create-internship", icon: "bi-briefcase" }
    ];
    ...
    return (
        <aside className={`app-sidebar d-flex flex-column text-white ${collapsed ? "collapsed" : ""} ${displayClass} ${mobileOpen ? "mobile-open" : ""}`} style={{ width, minWidth: width, zIndex: 1030 }}>
            <div className={`d-flex align-items-center p-3 ${collapsed && !mobileOpen ? "justify-content-center" : "justify-content-between"}`}>
                {(!collapsed || mobileOpen) && <div><h6 className="sidebar-brand mb-0">SkillBridgeAI</h6><small className="sidebar-subtitle text-secondary" style={{ fontSize: "0.7rem" }}>Admin Panel</small></div>}
