...
function Conversation() {
    ...
    useEffect(() => {
        ...
        const receiveMessage = (newMessage) => {
            if (newMessage.conversation === conversationId || newMessage.conversation?._id === conversationId) {
        ...
        <div className="d-flex flex-column" style={{ height: "calc(100vh - 120px)", minHeight: "480px" }}>
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-3 flex-shrink-0">
                <div className="card-body py-3 px-3 px-md-4 d-flex align-items-center justify-content-between">
                    <div className="d-flex align-items-center gap-3">
                        ...
                        <div className="bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: "46px", height: "46px" }}><i className="bi bi-person-workspace fs-5"></i></div>
                        <div>
                            <h6 className="fw-bold mb-0 d-flex align-items-center gap-2">Mentor <span className="d-none d-sm-inline text-muted fw-normal small">• Conversation</span></h6>
                            <small className="d-flex align-items-center gap-1" style={{ fontSize: "0.78rem" }}>
                                ...
                                <span className="text-muted d-none d-sm-inline">• Usually replies instantly</span>
                            </small>
                        </div>
                    </div>
            ...
            <div className="card border-0 shadow-sm rounded-4 flex-grow-1 d-flex flex-column overflow-hidden" style={{ minHeight: 0 }}>
                <div className="flex-grow-1 overflow-auto p-3 p-md-4" style={{ background: "#eef1f6", backgroundImage: "radial-gradient(circle at 1px 1px, rgba(0,0,0,0.04) 1px, transparent 0)", backgroundSize: "22px 22px" }}>
                    {loading && <div className="text-center py-5"><div className="spinner-border text-primary" role="status"></div><p className="text-muted small mt-2">Loading conversation...</p></div>}
                    {!loading && error && <div className="alert alert-danger py-2 small">{error}</div>}
                    {!loading && !error && messages.length === 0 && (
                        <div className="text-center py-5">
                            <div className="bg-white border rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style={{ width: "72px", height: "72px" }}><i className="bi bi-chat-heart text-primary fs-3"></i></div>
                            <h6 className="fw-bold">No messages yet</h6>
                            <p className="text-muted small mb-0">Say hello to your mentor and start the conversation.</p>
                        </div>
                    )}
                                    ...
                                    <div className={`d-flex ${mine ? "justify-content-end" : "justify-content-start"} mb-1`}>
                                        ...
                                        <div className={`position-relative px-3 py-2 shadow-sm ${mine ? "bg-primary text-white" : "bg-white border"}`} style={{ maxWidth: "min(72%, 320px)", borderRadius: mine ? "18px 18px 4px 18px" : "18px 18px 18px 4px" }}>
                                            ...
                                            <div className={`d-flex align-items-center gap-1 mt-1 ${mine ? "justify-content-end" : "justify-content-start"}`} style={{ fontSize: "0.68rem", opacity: mine ? 0.85 : 0.65 }}>
                                                ...
                                                {mine && <i className="bi bi-check2-all" style={{ fontSize: "0.85rem" }}></i>}
                                            </div>
                                        </div>
                                        {mine && <div className="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center flex-shrink-0 ms-2 align-self-end" style={{ width: "28px", height: "28px", fontSize: "0.7rem", fontWeight: 700 }}>You</div>}
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div className="bg-white border-top p-2 p-md-3 flex-shrink-0">
                    <form onSubmit={sendMessage} className="d-flex align-items-end gap-2">
                        ...
                        <button type="submit" className="btn btn-primary rounded-circle d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: "44px", height: "44px" }} disabled={sending || !message.trim()}>
