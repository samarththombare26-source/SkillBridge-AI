...
function MentorSidebar({ collapsed = false, mobileOpen = false, onToggle, onClose }) {
    const navigate = useNavigate(); const location = useLocation();
    const handleLogout = () => { localStorage.removeItem("token"); localStorage.removeItem("user"); navigate("/login"); };
    const isActive = (path) => path === "/mentor" ? location.pathname === "/mentor" : location.pathname.startsWith(path);
    const menuItems = [
        { name: "Dashboard", path: "/mentor", icon: "bi-speedometer2" },
        { name: "My Profile", path: "/mentor/profile", icon: "bi-person-circle" },
        { name: "Requests", path: "/mentor/requests", icon: "bi-inbox" },
        { name: "My Students", path: "/mentor/my-students", icon: "bi-people" }
    ];
    ...
    return (
        <aside className={`app-sidebar d-flex flex-column text-white ${collapsed ? "collapsed" : ""} ${displayClass} ${mobileOpen ? "mobile-open" : ""}`} style={{ width, minWidth: width, zIndex: 1030 }}>
            <div className={`d-flex align-items-center p-3 ${collapsed && !mobileOpen ? "justify-content-center" : "justify-content-between"}`}>
                {(!collapsed || mobileOpen) && <div><h6 className="sidebar-brand mb-0">SkillBridgeAI</h6><small className="sidebar-subtitle text-secondary" style={{ fontSize: "0.7rem" }}>Mentor Portal</small></div>}
            ...
            <div className="px-2 sidebar-scroll">
                {(!collapsed || mobileOpen) && <small className="sidebar-section px-2 opacity-75" style={{ fontSize: "0.7rem", letterSpacing: "0.08em", textTransform: "uppercase" }}>Mentor Menu</small>}
                <nav className="nav flex-column gap-1 mt-2">
                    {menuItems.map((item) => (
                        <Link key={item.path} to={item.path} onClick={mobileOpen ? onClose : undefined} className={`sidebar-link ${isActive(item.path) ? "active" : ""} ${collapsed && !mobileOpen ? "justify-content-center" : ""}`} title={collapsed && !mobileOpen ? item.name : undefined}>
                            <i className={`bi ${item.icon}`}></i><span className="sidebar-brand-text">{item.name}</span>
                        </Link>
                    ))}
                </nav>
            </div>
