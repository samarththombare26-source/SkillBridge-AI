...
function MainLayout() {
    ...
    const [collapsed, setCollapsed] = useState(() => localStorage.getItem("mainSidebarCollapsed") === "true");
    const [mobileOpen, setMobileOpen] = useState(false);
    useEffect(() => { (async () => { try { const res = await API.get("/profile"); setProfile(res.data.user); } catch {} })(); }, []);
    useEffect(() => { localStorage.setItem("mainSidebarCollapsed", collapsed); }, [collapsed]);
